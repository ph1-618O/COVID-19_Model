var monthToShow = "march"

function getData() {
    Promise.all([d3.csv("../static/data/state_cases.csv"),
            d3.csv("../static/data/state_deaths.csv"),
            d3.csv("../static/data/month_sum.csv"),
        ])
        .then(function (files) {
                var months = files[2];
                var cases = files[0];
                ...
                for(const keys of Object.keys(months)){
                    console.log(`${keys}`)
                }
                    // console.log(unpack(months, ))
